...
public class Player extends Mob implements ItemHolder, ClientTickable {
	...
	public static final int maxHealth = 30, maxStamina = maxStat, maxHunger = maxStat, maxWater = maxStat;
	...
	public int hunger, stamina, water, armor; // The current stats
	...
	private int waterStamCnt, stamWaterTicks; // Tiers of hunger penalties before losing a burger.
	private static final int maxWaterTicks = 400; // The cutoff value for stamHungerTicks
	private static final int[] maxWaterStams = { 10, 7, 5 }; // TungerStamCnt required to lose a burger.
	private static final int[] waterTickCount = { 120, 30, 10 }; // Ticks before decrementing stamHungerTicks.
	private static final int[] waterStepCount = { 8, 3, 1 }; // Steps before decrementing stamHungerTicks.
	private static final int[] minDehypoHealth = { 7, 6, 1 }; // Min hearts required for hunger to hurt you.
	private int waterChargeDelay; // The delay between each time the hunger bar increases your health
	private int waterDehypoDelay; // The delay between each time the hunger bar decreases your health
	...
	public Player(@Nullable Player previousInstance, InputHandler input) {
		...
		waterStamCnt = maxWaterStams[Settings.getIdx("diff")];
		stamWaterTicks = maxWaterTicks;
	...
	public void tick() {
		...
		if (stamina < maxStamina) {
			stamWaterTicks -= diffIdx; // Affect hunger if not at full stamina; this is 2 levels away from a hunger "burger".
			if (stamina == 0) stamWaterTicks -= diffIdx; // Double effect if no stamina at all.
		}
		...
		// This if statement encapsulates the hunger system
		if (!Bed.inBed(this)) {
			if (waterChargeDelay > 0) { // If the hunger is recharging health...
				stamWaterTicks -= 2 + diffIdx; // Penalize the hunger
				if (water == 0) stamWaterTicks -= diffIdx; // Further penalty if at full hunger
			}

			if (Updater.tickCount % Player.waterTickCount[diffIdx] == 0)
				stamWaterTicks--; // hunger due to time.

			if (stepCount >= Player.waterStepCount[diffIdx]) {
				stamWaterTicks--; // hunger due to exercise.
				stepCount = 0; // reset.
			}

			if (stamWaterTicks <= 0) {
				stamWaterTicks += maxHungerTicks; // Reset stamHungerTicks
				waterStamCnt--; // Enter 1 level away from burger.
			}

			while (waterStamCnt <= 0) {
				water--; // Reached burger level.
				waterStamCnt += maxHungerStams[diffIdx];
			}

			/// System that heals you depending on your hunger
			if (health < (baseHealth + extraHealth) && water > maxWater / 2) {
				waterChargeDelay++;
				if (waterChargeDelay > 20 * Math.pow(maxWater - hunger + 2, 2)) {
					health++;
					waterChargeDelay = 0;
				}
			} else waterChargeDelay = 0;

			if (waterDehypoDelay == 0) {
				waterDehypoDelay = 120;
			}

			if (water == 0 && health > minDehypoHealth[diffIdx]) {
				if (waterDehypoDelay > 0) waterDehypoDelay--;
				if (waterDehypoDelay == 0) {
					directHurt(1, Direction.NONE); // Do 1 damage to the player
				}
			}
	...
	public String getDebugWater() {
		return waterStamCnt + "_" + stamWaterTicks;
	}
