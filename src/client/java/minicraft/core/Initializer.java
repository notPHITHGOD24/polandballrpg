...
public class Initializer extends Game {
	...
	static void run() {
		...
		try {
            ...
            activity.assets().setSmallImage("logo");
			...
			Logging.GAMEHANDLER.debug("Failed to initialize Discord SDK, no discord running!");
			exception.printStackTrace();

		} catch (UnknownHostException exception) {
			Logging.GAMEHANDLER.warn("Failed to download Discord SDK, no internet connection!");
			exception.printStackTrace();

		} catch (Exception exception) {
			Logging.GAMEHANDLER.error("Unknown error");
			exception.printStackTrace();
		}
