...
public class Renderer extends Game {
	...
	private static void renderGui() {
		...
		if (!isMode("minicraft.settings.mode.creative")) {
			...
			for (int i = 0; i < Player.maxStat; i++) {
				...
				// Renders water!!!!!!!!!
				if (i < player.water) {
					screen.render(i * 8 + (Screen.w - 80), Screen.h - 32, 7, 0, 0, hudSheet.getSheet());
				} else {
					screen.render(i * 8 + (Screen.w - 80), Screen.h - 32, 7, 1, 0, hudSheet.getSheet());
				}
			}
		}
		...
		if (!player.isRemoved()) {
			if (AirWizard.active && (player.getLevel().depth == 1)) {
				...
				renderBossbar((int) ((((float) boss.health) / boss.maxHealth) * 100), "GERMANY!!!!");
			} else if (ObsidianKnight.active && (player.getLevel().depth == -4)) {
				ObsidianKnight boss = ObsidianKnight.entity;
				renderBossbar((int) ((((float) boss.health) / boss.maxHealth) * 100), "Austria?");
			} else if (Russia.active && (player.getLevel().depth == -4)) {
				ObsidianKnight boss = ObsidianKnight.entity;
				renderBossbar((int) ((((float) boss.health) / boss.maxHealth) * 100), "RUSSIA!!!!");
			}
		}
	...
	private static void renderDebugInfo() {
		...
		if (showDebugInfo) { // Renders show debug info on the screen.
			...
			info.add("Water stam: " + player.getDebugWater());
			if (player.armor > 0) {
