...
import minicraft.level.tile.PalmTile;
...
public class Level {
	...
	public final PalmTile.PalmType[] palmTypes;
	...
	public Level(int w, int h, long seed, int level, Level parentLevel, boolean makeWorld) {
		...
		palmTypes = new PalmTile.PalmType[w * h];
		{
			...
			for (int y = 0; y < h; y++) { // Loop through height
				for (int x = 0; x < w; x++) { // Loop through width
					...
					treeTypes[x + y * w] = (idx >= types.length || idx < 0) ? TreeTile.TreeType.OAK // Oak by default.
						: types[idx];
					palmTypes[x + y * w] = PalmTile.PalmType.OAK;
				}
			}
		}
	...
	private void checkRussia(boolean check) {
		if (depth == -2 && !Russia.beaten) { // Add the airwizard to the surface

			boolean found = false;
			if (check) {
				for (Entity e : entitiesToAdd)
					if (e instanceof Russia) {
						found = true;
						break;
					}
				for (Entity e : entities)
	...
	public void tick(boolean fullTick) {
		...
		while (entitiesToAdd.size() > 0) {
			...
			if (!inLevel) {
				if (Logging.logLevel)
					printEntityStatus("Adding ", entity, "furniture.DungeonChest", "mob.AirWizard", "mob.Russia", "mob.Player");
	...
	public void removeAllEnemies() {
		for (Entity e : getEntityArray()) {
			if (e instanceof EnemyMob)
			    ...
			    if (!(e instanceof Russia) || Game.isMode("minicraft.settings.mode.creative"))
					e.remove();
		}
	}
